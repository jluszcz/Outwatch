...
- `frontend/` — Preact + htm frontend source
    ...
    - `utils.js` — Pure helpers for labelling, sorting, and formatting seasons, names, watch offsets, and note bodies; shared with tests
...
- `shared/` — Code the Worker and the browser bundle both import, so the two never disagree
    ...
    - `reactions.js` — `REACTIONS`, the emoji set both the Worker (validation) and the browser (the picker) must agree on
...
- `PUT /api/posts/:post_id/reactions` — `{ emoji, on }`; adds or removes one of the emoji in `shared/reactions.js` on a note the caller can see. Idempotent in both directions, attributed to the individual's email rather than their column, and 400 for an emoji outside the set. Reacting to your own note is allowed
