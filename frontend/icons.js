...
// one path (bi-trash is two: the three lines and the can). `fill` is the -fill
// variant, used in dark mode where a 1px outline thins out against the dark
// surface; `null` means the icon has no variant worth swapping and the outline
// is used in both themes.
export const ICONS = {
    // bi-three-dots — the action menu trigger. No fill variant: it is three
    // solid dots in both directions already.
    dots: {
        outline: [
    ...
    // bi-sun / bi-moon and their fills — the theme toggle, which shows the
    // theme a click would switch *to*: a sun while dark is in effect, a moon
    // while light is. Paired with `filled={dark}` like every other icon here,
    // that means in practice only sun-fill and moon-outline ever paint — the
    // other two are the cost of these being ordinary ICONS entries rather than
    // a special case, and they are what would render if the toggle's logic
    // ever inverted.
    sun: {
        outline: [
...
// One glyph, sized in em so it rides its button's font-size, and painted with
// currentColor so it inherits the button's colour — including the red the
// delete item takes on hover — without a token of its own. Always aria-hidden:
// every caller labels the control it sits in.
export function Icon({ name, filled }) {
    const icon = ICONS[name];
    const paths = (filled && icon.fill) || icon.outline;
    return html`<svg
        class="bi"
        viewBox="0 0 16 16"
        fill="currentColor"
